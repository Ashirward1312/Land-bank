...
function FarmhouseCard({ item, index }) {
  ...
  return (
    <article className="group relative flex flex-col rounded-3xl bg-gradient-to-b from-slate-900/90 via-slate-950 to-black ring-1 ring-white/10 shadow-[0_20px_60px_-35px_rgba(0,0,0,1)] hover:ring-[#fde68a]/80 hover:shadow-[0_30px_90px_-45px_rgba(0,0,0,1)] hover:-translate-y-[2px] transition-all duration-200 overflow-hidden">
      ...
      {/* Option badge (kept absolute) */}
      <div
        className="absolute right-3 top-3 sm:right-4 sm:top-4 z-20 rounded-full bg-black/80 px-2.5 py-1 sm:px-3 text-[10px] sm:text-[11px] font-semibold text-slate-200 border"
      ...
      {/* ✅ IMPORTANT FIX:
          We add extra TOP + RIGHT padding so chips (PRICE ON REQUEST) never go under the badge */}
      <div className="p-4 sm:p-5 md:p-6 pt-12 sm:pt-6 pr-20 sm:pr-6 space-y-4 flex-1 flex flex-col min-w-0">
        {/* Tags */}
        <div className="flex flex-wrap gap-2 text-[10px] sm:text-[11px] font-semibold min-w-0">
          <span
          ...
          <span
            className="rounded-full px-3 py-1 text-black text-[10px] sm:text-[11px] font-semibold shadow-sm border"
            style={{
        ...
        <div className="space-y-1 min-w-0">
          <h2 className="text-base sm:text-lg md:text-xl font-bold text-white leading-snug uppercase break-words">
            {title}
          </h2>
          <p
        ...
        {/* Details box */}
        <div className="mt-1 rounded-2xl border border-white/5 bg-black/50 px-3 py-3 space-y-1.5 min-w-0">
      ...
      <div className="border-t border-white/5 px-4 sm:px-5 md:px-6 py-4 bg-black/70">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <a
            href="/#contact"
            className="inline-flex w-full sm:w-auto shrink-0 items-center justify-center rounded-full px-5 py-2 text-xs md:text-sm font-semibold text-neutral-900 shadow-lg ring-1 border transition duration-200"
            style={{
