...
/* -------------------- BIG LAND / PATCH WORK DATA -------------------- */

const PATCH_OPTIONS = [
  {
    id: "rasni-arang-20-40-acre",
    title: "20 TO 40 ACRES – NEAR RASNI AARANG ROAD",
    location: "Near Rasni – Aarang Road, Raipur",
    size: "Approx 20 to 40 acres",
    type: "Big Land ",
  },
  {
  id: "kachna-6-10-acres",
  title: "6 TO 10 ACRES – KACHNA, RAIPUR",
  location: "Kachna, Raipur",
  size: "Approx 6 to 10 acres",
  type: "Big Land",
},
{
  id: "vip-road-3-5-acres",
  title: "3 TO 5 ACRES – VIP ROAD, RAIPUR",
...
function PatchCard({ item, index }) {
  const { title, location, size, type } = item;
      ...
      <div className="border-t border-white/5 px-5 md:px-6 py-4 bg-black/70">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <span className="text-xs text-slate-400 sm:flex-1">
            Big land  near Rasni Aarang Road and Kachna Old
            Dhamtari Road – share your requirement.
          </span>
    ...
    <section className="relative bg-black text-slate-100 pt-24 pb-14 md:pt-28 md:pb-16">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {PATCH_OPTIONS.map((item, index) => (
            <PatchCard key={item.id} item={item} index={index} />
          ))}
        </div>
      </div>
    </section>
